...
const manutCursos = async (req, res) =>
  (async () => {
    ...
    const resp = await axios
      .get(process.env.SERVIDOR_DW3Back + "/getAllCursos", {
        headers: {
      ...
      .catch((error) => {
        ...
        res.render("cursos/view/vwManutCursos", {
  ...
  (async () => {
    if (req.method == "GET") {
      ...
      try {
        ...
        console.error(
          "Erro ao inserir dados no servidor backend:",
          error.message
        );
        res.json({
          status: "Error",
    ...
    try {
      if (req.method == "GET") {
        ...
        if (response.data.status == "ok") {
          ...
          response.data.registro[0].datanascimento = moment(
            response.data.registro[0].datanascimento
          ).format("YYYY-MM-DD");
  ...
  (async () => {
    ...
    try {
      if (req.method == "GET") {
        ...
        const token = req.session.token;
        console.log(
          "[ctlCursos|UpdateCurso] Valor regData:",
        ...
        try {
          ...
          console.error(
            "[ctlCursos.js|UpdateCurso] Erro ao atualiza dados de Cursos no servidor backend:",
            error.message
          );
          res.json({
            status: "Error",
...
const DeleteCursos = async (req, res) =>
  (async () => {
    ...
    const token = req.session.token;
    console.log(
      "[ctlCursos|DeleteCurso] Valor regData:",
      JSON.stringify(regData)
    );
    ...
    try {
      ...
      console.error(
        "[ctlCursos.js|DeleteCurso] Erro ao deletar dados de Cursos no servidor backend:",
        error.message
      );
      res.json({
        status: "Error",
